...
const ErrorBarsPlugin = {
    ...
    afterDatasetsDraw(chart, easingValue, options) {
        ...
        barchartCoords.forEach((dataset, i) => {
            ...
            dataset.forEach((bar, ii) => {
                let cur = errorBarCoords[i];

                if (!cur) {
                    return;
                }
                const hasLabelProperty = Object.hasOwnProperty.call(cur, bar.value.x);
                let errorBarData = null;

                // common scale such as categorical
                if (hasLabelProperty) {
                    errorBarData = cur[bar.value.x];
                } else if (!hasLabelProperty && cur[ii]) {
                    // hierarchical scale has its label property nested in b.label object as b.label.label
                    console.log('FROM CHARTJS')
                    console.log(cur[ii])
                    errorBarData = cur[ii];
                }
